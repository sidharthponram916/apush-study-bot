...
module.exports = {
    ...
    async execute(message, args) {
        let info_embed = new MessageEmbed()
            ...
            .setColor('#2729b8')
            .addFields(
                { name: `Test 3`, value: `Chapter Focus: 33,34,35` }
            );

        if (args[0] === 'info') return message.channel.send(info_embed);

        let questions = (await axios.get(`https://apush2940.sp43.repl.co/api/ap/${args[0]}`)).data;
        questions.sort(() => Math.random() - 0.5);

        if (args[1]) {
            questions.length = args[1];
        }
        ...
        for (question of questions) {
            ...
            const embed = new MessageEmbed()
                ...
                .setDescription(`${question.passage}`)
